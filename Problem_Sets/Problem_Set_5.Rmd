...
\frac{dSSE}{db_0} = & -2\sum_{i=1}^n (y_i - X_i^{T}b) = 0\\
0 = & \sum_{i=1}^n (y_i - X_i^{T}b) = \sum_{i=1}^n e_i\\
\end{align*}
...
>Therefore,
\begin{align*}
PZ = & X(X^{T}X)^{-1}X^{T}Z = (n*k) matrix
\end{align*}
This projection matrix P creates predicted values for the column vectors of Z with linear combinations of columns in X.
