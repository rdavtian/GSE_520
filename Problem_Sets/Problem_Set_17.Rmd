...
2SLS:
\   
$y = \rho_0 + \rho_1 \hat x + e$\      
$y = \rho_0 + \rho_1 (\pi_0 + \pi_1 z + \eta) + e$
...
>
$yrsexposure\_UR$ must be exogenous and relevant. Meaning it cannot be correlated with the error term of the regression and must have a nonzero correlation with $app\_funds\_pc50$. 
...
round(coeftest(mod3, vcov = vcovCR(mod3, data$statefip, "CR1")),5)
```

>
After implementing the two stage least squares, $app\_funds\_pc50$ is now significant at the 5% level with a p-value of 0.03 using $yrsexposure\_UR$ as the instrument. Adjusting for all other variables, a 1 unit increase in urban renewal funds per capita ($100) in 1950 is associated with an average increase of 2.4% in median familly income in 1980.
...
```
>
Since we observe an F-statistic approximately 10 for the weak instruments test with a p-value of 0.00188, there is enough evidence to suggest that $yrsexposure\_UR$ is a strong instrument. But, the Wu-Hausman test is shows less convincing results. The instrument is only significant at the $10\%$ significance level. This suggests we may have a potentially weak instrument in $yrsexposure\_UR$.
