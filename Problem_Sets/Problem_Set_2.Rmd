...
>**Solution:**  
We can interpret the intecept $120$ as the expected mean quantity given that both price and income are zero.

b. What is the interpretation of the number $9.8$ in the equation above?  Is this a marginal effect?  If so, of what?

>Adjusting for income, we can expect quantity to decrease 9.8 units on average for every additional dollar increase in price. 
...
>Clearly, demand cannot be negative. Using linear functions to describe demand curves can yield outputs (quantity) values that are not in the range of possible values given some input values (price, income). 
...
>**Solution:**
We can interpret the intercept $83.1$ as the expected mean proportion of employees partcipating in a 401k given that the employer's match rate is 0.
...
>**Solution:**  
Yes, it makes sense that the coefficient on $mrate$ is positive. As the match rate increases, we expect more people to participate in a 401k. Companies matching a higher percentage of salary means employees will have more money in their 401k which would entice people to join.   
