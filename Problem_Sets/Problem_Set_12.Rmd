...
merged <- studysample_allmerged %>% 
  ...
  mutate(Utreat03v1 = as.factor(Utreat03v1),
         HIVtreat03v1 = as.factor(HIVtreat03v1),
         HandUtreat03v1 = as.factor(HandUtreat03v1),
...
vars <- c("Utreat03v1","HIVtreat03v1","HandUtreat03v1","yrbirth","Q_year","schsize","stratum","age03","reached_8th_grade","weight_sample")
males2 <- males[, vars] %>% na.omit()
females2 <- females[, vars] %>% na.omit()

mod_males_8th_grade <- lm(reached_8th_grade ~ Utreat03v1 + HIVtreat03v1 + HandUtreat03v1 + yrbirth + Q_year + schsize + stratum + age03, 
data = males2, weights = weight_sample)
mod_females_8th_grade <- lm(reached_8th_grade ~ Utreat03v1 + HIVtreat03v1 + HandUtreat03v1 + yrbirth + Q_year + schsize + stratum + age03, 
data = females2, weights = weight_sample)
```

```{r}
# Education Years as Response
vars <- c("Utreat03v1","HIVtreat03v1","HandUtreat03v1","yrbirth","Q_year","schsize","stratum","age03","Q_b1_13_school_years","weight_sample")
males2 <- males[, vars] %>% na.omit()
females2 <- females[, vars] %>% na.omit()
mod_males_educ_years <- lm(Q_b1_13_school_years ~ Utreat03v1 + HIVtreat03v1 + HandUtreat03v1 + yrbirth + Q_year + schsize + stratum, 
                          data = males2, weights = weight_sample)
mod_females_educ_years <- lm(Q_b1_13_school_years ~ Utreat03v1 + HIVtreat03v1 + HandUtreat03v1 + yrbirth + Q_year + schsize + stratum, 
                            data = females2, weights = weight_sample)
```

```{r}
# Ever Married as Response
vars <- c("Utreat03v1","HIVtreat03v1","HandUtreat03v1","yrbirth","Q_year","schsize","stratum","age03","ever_married","weight_sample")
males2 <- males[, vars] %>% na.omit()
females2 <- females[, vars] %>% na.omit()
mod_males_ever_married <- lm(ever_married ~ Utreat03v1 + HIVtreat03v1 + HandUtreat03v1 + yrbirth + Q_year + schsize + stratum, 
                           data = males2, weights = weight_sample)
mod_females_ever_married <- lm(ever_married ~ Utreat03v1 + HIVtreat03v1 + HandUtreat03v1 + yrbirth + Q_year + schsize + stratum, 
                             data = females2, weights = weight_sample)
```

```{r}
# Ever Pregnant as Response
vars <- c("Utreat03v1","HIVtreat03v1","HandUtreat03v1","yrbirth","Q_year","schsize","stratum","age03","ever_pregnant","weight_sample")
males2 <- males[, vars] %>% na.omit()
females2 <- females[, vars] %>% na.omit()
mod_males_ever_pregnant <- lm(ever_pregnant ~ Utreat03v1 + HIVtreat03v1 + HandUtreat03v1 + yrbirth + Q_year + schsize + stratum, 
                             data = males2, weights = weight_sample)
mod_females_ever_pregnant <- lm(ever_pregnant ~ Utreat03v1 + HIVtreat03v1 + HandUtreat03v1 + yrbirth + Q_year + schsize + stratum, 
                               data = females2, weights = weight_sample)
```
