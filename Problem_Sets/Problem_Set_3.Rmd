...
>In Question 2 part c, we used the model lsalary = a + b(lmktval) as a linear regression model that gave an $R^{2}$ value of 0.232. In this problem, we used the multiple linear regression model lsalary = a + b(lsales) + c(lmktval) + d(profmarg) + e(comtensq) + f(ceoten) which gave an $R^{2}$ value of 0.354. These five variables together explain more of the variation in log of salary than the previous one variable model.     
